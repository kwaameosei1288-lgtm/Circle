        ...
        if (!supabase) {
            ...
            try {
                if (window.supabase && window.config) {
                    ...
                    if (SUPABASE_URL && SUPABASE_ANON_KEY) {
                        supabase = window.supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY);
                        console.log('✓ Supabase initialized synchronously');
                    }
                }
            } catch (error) {
                console.error('Failed to initialize Supabase synchronously:', error);
            }
            
            // If still not initialized, try async initialization
            if (!supabase) {
                console.warn('Supabase not initialized yet, attempting async initialization...');
                initSupabase().catch(error => {
